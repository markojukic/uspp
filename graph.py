...
class GraphReader(Conn):
    ...
    def to_pandas(self):
        return pd.read_sql('SELECT * FROM vertices', self.conn, index_col='id'), pd.read_sql('SELECT * FROM edges',
                                                                                             self.conn)
    ...
    def neighbourhood(self, value: str, method=1, f=None) -> Tuple[pd.Series, csr_matrix]:
        ...
        if method == 1 or method == 3:
            self.conn.execute(
                'CREATE TEMPORARY VIEW IF NOT EXISTS {1} AS SELECT id1 AS id FROM edges WHERE id2={0} UNION SELECT id2 FROM edges WHERE id1={0} UNION VALUES({0})'.format(
                    w, vertices_view))
        elif method == 2:
        ...
        return vertices.set_index('i')['value'], csr_matrix(
